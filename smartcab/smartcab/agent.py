...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.5):
        ...
        self.q_template = dict((k, 0.0) for k in self.valid_actions)

        for state_tuple in itertools.product(*self.state_def):
            self.Q[state_tuple] = self.q_template.copy()
    ...
    def reset(self, destination=None, testing=True):
        ...
        if testing:
            ...
            # Use negative exponential e^(-a*t) decay function
            self.epsilon = math.exp(-self.alpha*self.time)
    ...
    def createQ(self, state):
        ...
        if self.learning:
            if not state in self.Q:
                self.Q[state] = self.q_template.copy()
...
def run():
    ...
    sim.run(n_test=100,tolerance=.01)
